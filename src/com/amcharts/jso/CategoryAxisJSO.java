...
public final class CategoryAxisJSO extends AxisBaseJSO implements IsCategoryAxis
{
	...
	public final native Boolean isBoldPeriodBeginning() /*-{
		return this.boldPeriodBeginning;
	}-*/;
	...
	/**
	 * "specifies a method that returns the value that should be used as categoryValue for current item. If this property is set, the return this.value of the custom data function takes precedence over categoryField. When a chart calls this method, it passes category value, data item from chart's data provider and reference to categoryAxis: categoryFunction(category, dataItem, categoryAxis); This method can be used both when category axis parses dates and when it doesn't. If axis parses dates, your categoryFunction should return this.Date object. For example, if you have date strings in your data, you can use this method to convert these strings into Date objects."""
	 */
	...
	public final native Boolean isCenterLabelOnFullPeriod() /*-{
		return this.centerLabelOnFullPeriod;
	}-*/;
	...
	/**
	 * "You can use this function to format Category axis labels. If this function is set, then it is called with the following parameters passed: if dates are not parsed: labelFunction(valueText, serialDataItem, categoryAxis) if dates are parsed: labelFunction(valueText, date, categoryAxis) Your function should return this.string which will be displayed on the axis."
	 */
